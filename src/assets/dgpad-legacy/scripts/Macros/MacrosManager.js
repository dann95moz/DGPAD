...
function MacrosManager(_canvas) {
    ...
    me.getCatalog = function() {
        ...
        for (var i = 0; i < plugins.length; i++) {
            if (hidden.indexOf(plugins[i].id) === -1) pluginItems.push({ key: "plugin:" + i, name: plugins[i].name });
        }
        for (var j = 0; j < tools.length; j++) toolItems.push({ key: "tool:" + j, name: tools[j].name });
    ...
    me.startExternal = function(_key) {
        ...
        if (!macro) return;
        activeExternal = { key: _key, name: macro.shortname, prompt: "", types: macro.getParamTypes ? macro.getParamTypes() : [] };
        var label = {};
        Object.defineProperty(label, "innerHTML", {
            set: function(_html) {
                if (!activeExternal || activeExternal.key !== _key) return;
                var plain = String(_html).replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim();
                if (plain.indexOf(macro.shortname) === 0) plain = plain.substring(macro.shortname.length).replace(/^\s*:\s*/, "");
    ...
    me.getActiveExternal = function() {
        ...
        return { key: activeExternal.key, name: activeExternal.name, prompt: activeExternal.prompt, types: activeExternal.types.slice() };
    };
