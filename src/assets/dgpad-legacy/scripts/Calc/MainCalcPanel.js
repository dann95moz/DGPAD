function MainCalcPanel(_man, _canvas) {
    ...
    me.ensureExternalObject = function() {
        if (OBJ === null) me.createObj();
    };

    me.getExternalState = function() {
        return {
            editing: OBJ !== null,
            e1: E1.getText(), e1Label: E1.getLabel(),
            e2: E2.getText(), e2Label: E2.getLabel(),
            min: MIN.getText(), max: MAX.getText(),
            showE2: !E2.isHidden(), showMin: !MIN.isHidden(), showMax: !MAX.isHidden(),
            degrees: !!Cn.isDEG()
        };
    };

    me.setExternalField = function(_field, _value) {
        var fields = { e1: E1, e2: E2, min: MIN, max: MAX };
        if (fields[_field]) fields[_field].setText(_value);
    };

    me.setExternalDegrees = function(_value) {
        Cn.setDEG(!!_value); Cn.computeAll(); canvas.paint();
    };
