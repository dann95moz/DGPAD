...
function props_gridPanel(_owner) {
    ...
    var sFontAxis = new slider(me.getDocObject(), 10, ch, 200, 40, 6, 60, CS.getFontSize(), FONTAXIScallback);
    sFontAxis.setValueWidth(40);
    sFontAxis.setLabel($L.props_font, 110);
    sFontAxis.setTextColor("#252525");
    sFontAxis.setValuePrecision(1);
    sFontAxis.setBackgroundColor("rgba(0,0,0,0)");
    let _FontAxisStart = null;
    sFontAxis.setOnDragStart((startVal) => { _FontAxisStart = startVal; });
    sFontAxis.setOnDragEnd((startVal, endVal) => {
    if (startVal !== endVal) {
        canvas.undoManager.recordPropertyChange(CS, 'AxisFont', startVal, endVal);
    }
    });
    ch += 40;
    ...
    let _axisStart = null;
    sAxis.setOnDragStart((startVal) => { _axisStart = startVal; });
    sAxis.setOnDragEnd((startVal, endVal) => {
    if (startVal !== endVal) {
        canvas.undoManager.recordPropertyChange(CS, 'AxisWidth', startVal, endVal);
    }
    });
    ch += 40;
...
function props_namePanel(_owner) {
    ...
    var show_callback = function(_val) {
        ...
        canvas.undoManager.recordPropertyChange(me.obj, 'ShowName', old, _val);
...
function props_colorPanel(_owner) {
    ...
    var HEXcallback = function(_hex) {
        if (setall)
            ...
            canvas.undoManager.recordPropertyChange(me.obj, 'Color', old, _hex);
    ...
    var FONTcallback = function(_val) {
        if (setall)
            ...
            canvas.undoManager.recordPropertyChange(me.obj, 'FontSize', old, _val);
    ...
    var PSHAPEcallback = function(_val) {
        if (setall)
            ...
            old = me.obj.getShape();
            canvas.undoManager.recordPropertyChange(me.obj, 'Shape', old, _val);
    ...
    var DSHcallback = function(_val) {
        if (setall)
            ...
            old = me.obj.isDash();
            canvas.undoManager.recordPropertyChange(me.obj, 'Dash', old, _val);
            me.obj.setDash(_val);
        me.repaint();
    };
    ...
    var NOMOUSEcallback = function(_val) {
        if (setall)
            ...
            canvas.undoManager.recordPropertyChange(me.obj, 'NoMouseInside', old, _val);
    ...
    me.setObj = function() {
        ...
        let _sizeStart = null;
        sSize.setOnDragStart((startVal) => {
        // puedes usar startVal o leer del objeto; usa uno solo para consistencia
        _sizeStart = startVal; // me.obj.getSize();
        });
        sSize.setOnDragEnd((startVal, endVal) => {
        // registra UN SOLO paso si realmente cambió
        if (startVal !== endVal) {
            canvas.undoManager.recordPropertyChange(me.obj, 'Size', startVal, endVal);
            // console.log('registró Size', startVal, '→', endVal);
        }
        });
        ...
        if (me.obj.getCode() === "list") {
            ...
            let _segSizeStart;
            segSize.setOnDragStart((startVal) => { _segSizeStart = startVal });
            segSize.setOnDragEnd((startVal,endVal) => {
            if (startVal !== endVal) {    
                canvas.undoManager.recordPropertyChange(me.obj, 'SegmentsSize', startVal,endVal);
            }
            });
        ...
        let _opacityStart = null;
        sOpacity.setOnDragStart((startVal) => {
        // puedes usar startVal o leer del objeto; usa uno solo para consistencia
        _opacityStart = startVal; // me.obj.getSize();
        });
        sOpacity.setOnDragEnd((startVal, endVal) => {
        // registra UN SOLO paso si realmente cambió
        if (startVal !== endVal) {
            canvas.undoManager.recordPropertyChange(me.obj, 'Opacity', startVal, endVal);
            // console.log('registró Size', startVal, '→', endVal);
        }
        });
        ...
        let _layerStart=null;
        sLayer.setOnDragStart(() => { _layerStart = startVal });
        sLayer.setOnDragEnd((startVal, endVal) => {
        if (startVal !== endVal) {
            canvas.undoManager.recordPropertyChange(me.obj, 'Layer', startVal,endVal);
        }
        });
        ...
        let _fontStart=null;
        sFont.setOnDragStart((startVal) => { _fontStart = startVal });
        sFont.setOnDragEnd((startVal,endVal) => {
        if (startVal !== endVal) {
            canvas.undoManager.recordPropertyChange(me.obj, 'FontSize', startVal,endVal);
        }
        });
        ...
        let _precStart;
        sPrec.setOnDragStart((startVal) => { _precStart = startVal });
        sPrec.setOnDragEnd((startVal,endVal) => {
        if (startVal !== endVal) {    
            canvas.undoManager.recordPropertyChange(me.obj, 'Precision', startVal,endVal);
        }
        });
        ...
        if (me.obj.getCode() === "angle") {
            ...
            ch += sh;
            let _incStart=null;
            sInc.setOnDragStart((startVal) => { _incStart = startVal });
            sInc.setOnDragEnd((startVal,endVal) => {
            if (startVal !== endVal) {    
                canvas.undoManager.recordPropertyChange(me.obj, 'Increment', startVal,endVal);
            }
            });
