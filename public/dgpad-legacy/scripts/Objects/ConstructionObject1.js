...
function ConstructionObject(_construction, _name) {
  ...
  this.setDash = function(_d) {
    const old = this.isDash();
    Cn.canvas.undoManager.recordPropertyChange(this, 'Dash', old, _d);
  ...
  this.setPrecision = function(_prec) {
    const old = this.getPrecision();
    Cn.canvas.undoManager.recordPropertyChange(this, 'Precision', old, _prec);
    if (_prec > -1) {
  ...
  this.setLayer = function(_l) {
    const old = this.getLayer();
    Cn.canvas.undoManager.recordPropertyChange(this, 'Layer', old, _l);
    layer = _l;
  ...
  this.setFontSize = function(_s) {
    const old = this.getFontSize();
    Cn.canvas.undoManager.recordPropertyChange(this, 'FontSize', old, _s);
    fontsize = _s;
  };
  ...
  this.setName = function(_n) {
    ...
    Cn.canvas.undoManager.recordPropertyChange(this, 'Name', old, _n);
    this.setNameOnly(_n);
  ...
  this.setShowName = function(_bool) {
    const old = this.getShowName();
    Cn.canvas.undoManager.recordPropertyChange(this, 'ShowName', old, _bool);
    showname = _bool;
  ...
  //   if (Cn.getFrame && typeof Cn.getFrame().updateTextCons === 'function') {

  //     Cn.getFrame().updateTextCons(this);  // Actualizar el protocolo para este objeto
  //     console.log("actualizó")
  //   }
  // };

  this.setHidden = function(_sel) {
    const old = this.isHidden();
    Cn.canvas.undoManager.recordPropertyChange(this, 'Hidden', old, _sel);
    _sel = Math.abs(_sel * 1);
    hidden = (isNaN(_sel)) ? 1 : parseInt(_sel) % 3;
    this.paint = paintTab[hidden];
    this.validate = validTab[hidden];
  ...
  this.setColor = function(_col) {
    ...
    Cn.canvas.undoManager.recordPropertyChange(this, 'Color', old, _col);
    color.set(_col);
  ...
  this.setOpacity = function(_f) {
    const old = this.getOpacity();
    Cn.canvas.undoManager.recordPropertyChange(this, 'Opacity', old, _f);
  ...
  this.setSize = function(_s) {
    const old = this.getSize();
    Cn.canvas.undoManager.recordPropertyChange(this, 'Size', old, _s);
    size = _s;
  };
  ...
  this.setProtected = function(val) {
    const old = this.isProtected();
    Cn.canvas.undoManager.recordPropertyChange(this, 'Protected', old, val);
    this.protected = !!val;
  };
