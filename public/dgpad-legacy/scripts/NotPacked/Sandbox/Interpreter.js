...
// if (!window.$U.inputValues) window.$U.inputValues = {};
...
//       window.$U.inputValues[message.id] = message.value;
      
//     //   SetExpressionValue(message.expression, parseFloat(message.value));
...
window.addEventListener("message", function (e) {
  ...
  switch (message.action) {
    ...
    // const OP_INPUT = 91; // el mismo opcode que usas para registrar inputs en TAB
    ...
    //     if (Array.isArray(cmd) && cmd[0] === OP_INPUT && cmd[3] && cmd[3].point === oldN) {
    //     cmd[3].point = newN;
    //     }
    // }
    // break;
    // }
    // /interpreter/message-listener.js (IFRAME) — opcional pero recomendado
// En tu switch(message.action):
// case "rename-point-for-tab":
// Actualiza ambos opcodes: texto (si usas 91) y número (92)
    case "rename-point-for-tab": {
    const oldN = String(message.oldName || "");
    const newN = String(message.newName || "");
    if (!oldN || !newN || oldN === newN) break;
    ...
    //   const id = String(message.id || "");
    //   if (!id) break;
    //   delete window.$U.inputValues[id];
    //   if (window.$U.inputs) delete window.$U.inputs[id];
    //   break;
    // }
    case "delete-input":
        delete window.$U.inputValues[message.id];
...
function crearCronometro(id) {
  ...
  if (!cronometros[id]) {
    ...
    console.log("✅ Cronómetro creado en sandbox:", id);
  } else {
