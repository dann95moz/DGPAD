function CalcManager(_canvas) {
    ...
    var notifyParent = function() {
        if (externalPanel && window.parent !== window) {
            window.parent.postMessage({ type: "dgpad-calculator-state" }, window.location.origin);
        }
    };
    ...
    me.getExternalState = function() {
        return maincalc ? maincalc.getExternalState() : null;
    };

    me.ensureExternalObject = function() {
        if (maincalc) {
            maincalc.ensureExternalObject();
            notifyParent();
        }
    };

    me.setExternalField = function(_field, _value) {
        if (maincalc) {
            maincalc.setExternalField(_field, _value);
            notifyParent();
        }
    };

    me.setExternalDegrees = function(_value) {
        if (maincalc) maincalc.setExternalDegrees(_value);
        notifyParent();
    };

    me.validateExternal = function() {
        if (maincalc) maincalc.valid();
        notifyParent();
    };

    me.cancelExternal = function() {
        if (maincalc) maincalc.cancel();
        notifyParent();
    };
