   function BlocklyManager(_canvas) {
       ...
       var initBlockly = function() {
           ...
           Blockly.dgpad = new function() {
            ...
            //        var props = me.CN.getAllObjectsFromType(_t);
            //        var tab = [];
            ...
            me.popupArray = function(_t) {
                ...
                for (var i = 0; i < props.length; i++) {
                    if ((mod !== "oncompute") || (OBJ != props[i])) {
                        ...
                        // 🧠 Traducir nombres internos a nombres legibles
                        if (internalName.startsWith("blk_btn")) {
                            displayName = $L.blockly.expressions_button + internalName.substring(7); // blk_btn0 → boton0
                        } else if (internalName.startsWith("blk_turtle_exp_")) {
                            displayName = $L.blockly.expressions_trtlExp + internalName.substring(15); // blk_turtle_exp_P1 → expresion_tortuga_P1
                        } else if (internalName.startsWith("blk_turtle_list_")) {
                            displayName = $L.blockly.expressions_trtlList + internalName.substring(15); // blk_turtle_list_P2 → lista_tortuga_P2
                        }
       ...
       var showCategory = function(name, bool) {
           ...
           var cat = { "turtle": 7, "texts": 8, "inputs": "c" , "geometry":"b"};
    ...
    var onchanged = function(e) {
                ...
                if (e.oldXml) {
                    ...
                    for (let i = 0; i < blocks.length; i++) {
                        ...
                        if (deletableInputBlocks.includes(blockType)) {
                            const blockId = block.getAttribute("id");
                            // const inputId = `inputP1-input_${blockId}`;
                            const inputId = `P1-input_${blockId}`;
       ...
       var showCurrentTab = function() {
           ...
           setTimeout(function() {
               ...
               showCategory("geometry", (mod === "onprogram"));
           }, 300)
       };
